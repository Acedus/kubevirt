...
func sortedContextsByOffset(lastExtents map[string]*nbdv1.Extent) []string {
	...
	sort.Slice(contexts, func(i, j int) bool {
		ei := lastExtents[contexts[i]]
		ej := lastExtents[contexts[j]]
		switch {
		case ei == nil && ej == nil:
...
func (c *NBDClient) GetExportInfo(ctx context.Context, req *nbdv1.ExportInfoRequest) (*nbdv1.ExportInfoResponse, error) {
	l, err := libnbd.Create()
	if err != nil {
		return nil, fmt.Errorf("failed to create nbd handle: %w", err)
	}
	defer l.Close()
...
func (c *NBDClient) GetMap(req *nbdv1.MapRequest, stream nbdv1.NBD_GetMapServer) error {
	l, err := libnbd.Create()
	if err != nil {
...
func resolveReadRange(offset, length, size uint64) (uint64, error) {
	if offset >= size {
...
func (c *NBDClient) Read(req *nbdv1.ReadRequest, stream nbdv1.NBD_ReadServer) error {
	...
	length, err := resolveReadRange(req.Offset, req.Length, size)
	if err != nil {
		return err
	}
...
func resolveRange(offset, length, size uint64) (current, end uint64, err error) {
	if offset >= size {
		return 0, 0, fmt.Errorf("offset %d exceeds export size %d", offset, size)
	}
	if length == 0 || offset+length > size {
		length = size - offset
	}
