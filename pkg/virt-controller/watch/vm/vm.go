...
func (c *Controller) createVMRevision(vm *virtv1.VirtualMachine) (string, error) {
	...
	cr, err = c.clientset.AppsV1().ControllerRevisions(vm.Namespace).Create(context.Background(), cr, metav1.CreateOptions{})
	if err != nil {
		return "", err
	}
...
func (c *Controller) updateStatus(vm, vmOrig *virtv1.VirtualMachine, vmi *virtv1.VirtualMachineInstance, syncErr common.SyncError, logger *log.FilteredLogger) error {
	...
	if created {
		...
		vmi, err = c.bumpObservedGeneration(vm, vmi)
		if err != nil {
			return err
		}
	}
...
func (c *Controller) bumpObservedGeneration(vm *virtv1.VirtualMachine, vmi *virtv1.VirtualMachineInstance) (*virtv1.VirtualMachineInstance, error) {
	...
	if equality.Semantic.DeepEqual(vm.Spec.Template.Spec, revisionSpec.Template.Spec) {
		return vmi, nil
	}
...
func shouldCreateNewRevision(vmSpec, revisionSpec virtv1.VirtualMachineSpec) bool {
	var createNewRevision bool
	if !equality.Semantic.DeepEqual(vmSpec.Template.Spec, revisionSpec.Template.Spec) {
		createNewRevision = true
	}
	if vmSpec.RunStrategy != revisionSpec.RunStrategy {
		createNewRevision = true
	}
