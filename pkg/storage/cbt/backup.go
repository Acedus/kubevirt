...
func (ctrl *VMBackupController) setFailed(backup *backupv1.VirtualMachineBackup, failureReason, message string) {
	meta.SetStatusCondition(&backup.Status.Conditions, metav1.Condition{
		Type: string(backupv1.ConditionFailed), Status: metav1.ConditionTrue,
		Reason: failureReason, Message: fmt.Sprintf(backupFailed, message),
	})
	markTerminal(backup, failureReason, fmt.Sprintf(backupFailed, message))
