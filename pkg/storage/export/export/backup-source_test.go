...
var _ = Describe("Backup source", func() {
	...
	BeforeEach(func() {
		...
		fakeBackupCertManager, err := bootstrap.NewMockCertificateManager()
		...
		controller = &VMExportController{
			...
			BackupCertManager:           fakeBackupCertManager,
	...
	It("Should create VM export and service when backup is progressing", func() {
		...
		k8sClient.Fake.PrependReactor("create", "services", func(action testing.Action) (handled bool, obj runtime.Object, err error) {
			...
			Expect(portNames).To(ContainElements("export", "backup"))
			return true, svc, nil
		})
		...
		cert, err := controller.backupPublicKeyPEM()
		Expect(err).ToNot(HaveOccurred())

		Expect(pod.Spec.Containers[0].Env).To(ContainElements(
			k8sv1.EnvVar{Name: "BACKUP_PUBLIC_KEY", Value: cert},
	...
	It("Should return error when BackupCertManager is not initialized", func() {
		controller.BackupCertManager = nil
		...
		Expect(err).To(HaveOccurred())
		Expect(err.Error()).To(ContainSubstring("BackupCertManager is not initialized"))
		Expect(retry).To(BeEquivalentTo(0))
	})
})
